...
public abstract class ServerAnswers {
     ...
     */
    public static ServerCommand doCommand(ServerCommand command, ArrayList<Ticket> tickets, ZonedDateTime date) {
        ServerCommand answer;
        switch (command.getCommand()) {
            case add:
                answer = add_(tickets, command.getInformation());
                break;
            case add_if_max:
                answer = add_if_max_(tickets, command.getInformation());
                break;
            case add_if_min:
                answer = add_if_min_(tickets, command.getInformation());
                break;
            case clear:
                answer = clear_(tickets);
                break;
            ...
            case info:
                ...
                break;
            case remove_by_id:
                answer = remove_by_id_(tickets, command.getInformation());
                break;
            case remove_lower:
                answer = remove_lover_(tickets, command.getInformation());
                break;
            case show:
            ...
            case update:
                ...
                break;
            default:
                answer = new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s", "Команда не обнаружена"));
        }
        return answer;
    }

    private static ServerCommand add_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        tickets.add(add_basic(inf));
        return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Элемент был успешно добавлен"));
    }

    private static ServerCommand add_if_max_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        Ticket ticket = add_basic(inf);
        if (tickets.stream().anyMatch(a -> a.compareTo(ticket) >= 0))
            return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Ваш элемент не был добвлен, так как он не является наибольшим"));
        else {
            tickets.add(ticket);
            return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Элемент был успешно добавлен"));
        }
    ...
    private static ServerCommand add_if_min_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        Ticket ticket = add_basic(inf);
        if (tickets.stream().anyMatch(a -> a.compareTo(ticket) <= 0))
            return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Ваш элемент не был добвлен, так как он не является наименьшим"));
        else {
            tickets.add(ticket);
            return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Элемент был успешно добавлен"));
        }
    ...
    private static ServerCommand clear_(ArrayList<Ticket> tickets) {
        tickets.clear();
        return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Коллкция была очищена"));
    }
    ...
    private static ServerCommand count_greater_then_type_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        ...
        return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s",
    ...
    private static ServerCommand info_(ArrayList<Ticket> tickets, ZonedDateTime date) {
        return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Коллкция типа " +
                tickets.getClass().getSimpleName() +
                " содержит " + tickets.size() +
                " элементов и была создана " + date));
    }

    private static ServerCommand remove_by_id_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        int id = Integer.parseInt(inf.get(0));
        return new lab_6.common.ServerCommand(AllCommands.service, tickets.removeIf(a -> a.getId() == id) ?
                String.format("%n%s%n", "Из коллекции был удалён элемент с ID" + id) :
                String.format("%n%s%n", "Элемент с ID" + id + " не был найден"));

    }

    private static ServerCommand remove_lover_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        ...
        List<Ticket> found_tickets = tickets.stream().filter(a -> a.compareTo(test_ticket) < 0).collect(Collectors.toList());
        tickets.removeAll(found_tickets);
        return new ServerCommand(AllCommands.service,
                String.format("%n%s%n", found_tickets.isEmpty() ? "Подходящих под критерии элементов не нашлось, ни один элемент удалён не был" :
    ...
    private static ServerCommand show_(ArrayList<Ticket> tickets) {
        ...
        return new lab_6.common.ServerCommand(AllCommands.service, String.format(format.toString(), answerinf.toArray()));
    }
    ...
    private static ServerCommand update_(ArrayList<Ticket> tickets, ArrayList<String> inf) {
        if (inf.get(0).isEmpty())
            return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s", "Элемент не был изменён"));
        else {
            ...
            try {
                ...
                return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Данные введены некоректно, элемент не был изменён"));
            }
            return new lab_6.common.ServerCommand(AllCommands.service, String.format("%n%s%n", "Элемент был успешно изменён"));
        }
    }

    private static Ticket add_basic(ArrayList<String> inf) {
