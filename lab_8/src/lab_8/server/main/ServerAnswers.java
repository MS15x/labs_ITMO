    ...
    */
    private static ServerCommand update_(ArrayList<Ticket> tickets, ArrayList<String> inf, String user) {
        ...
        try {
            if (ticket != null && ticket.getUser().equals(user))
                switch (Integer.parseInt(inf.get(1))) {
                    ...
                    case 8:
                        ...
                        else ticket.getEvent().setName(inf.get(2));
                        break;
                    case 9:
                        if (ticket.getEvent() != null)
            ...
            return new ServerCommand(AllCommands.service, "Данные введены некоректно, элемент не был изменён");
        }
        return new ServerCommand(AllCommands.update, "ok", inf.get(0), inf.get(1), inf.get(2));
    }
